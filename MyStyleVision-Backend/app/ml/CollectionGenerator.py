...
import os
import random
from .utils.models import call_openai, call_gpt_4_turbo
...
class CollectionGenerator:
    ...
    @staticmethod
    def generate_collection_image(collection_description: str):
        """ Generate an image for a collection based on its description. """
        client = CollectionGenerator.get_openai_client()
        prompt = f"""CONTEXT:
        You are a fashion designer and you are assigned with the task of designing a new collection for the upcoming season.
        The following is a description of the collection: \n {collection_description} \n\n. You are tasked with 
        designing a cover image for this collection. Keep the image simple, elegant, and visually appealing.
        The image should capture the essence of the collection and entice customers to explore further. 
        The image should be in a portrait orientation and should be suitable for use on a website or social
        media platform. \n\n DIRECTIONS: Design a cover image for the collection based on the description
        provided. The image should be visually appealing and should reflect the theme and style of the 
        collection. Use a plain background and focus on the key elements of the collection. The image
        ...
        try:
            ...
            return process_and_upload_image(url, "mystylevision-images")
